...
import {
  createFoodPage,
  updateFoodPage,
} from "../../../redux/actions/foodPageaction";
...
const HomePageForm = ({ setupdateFoodCurrentId, updateFoodCurrentId }) => {
  ...
  const UpdateFooPage = foodPageData.filter(
    updatefoodData => updatefoodData._id === updateFoodCurrentId
  )[0];
  React.useEffect(() => {
    if (UpdateFooPage) {
  ...
  const handleSubmit = async e => {
    ...
    if (updateFoodCurrentId) {
      dispatch(
        updateFoodPage(updateFoodCurrentId, {
          ...postData,
      ...
      >
        <Grid container spacing={3} item sm={6} md={6}>
          <StyleTextField
            ...
            onChange={e =>
              setPostData({ ...postData, title: e.target.value.toLowerCase() })
            }
        ...
        <Grid container spacing={3} item sm={6} md={6}>
          ...
          {progress ? (
            ...
            <div style={{ textAlign: "center" }}>
              <input
                style={{ padding: "20px 0px", marginLeft: "50px" }}
                type="file"
                id="selectedFile"
                name="selectedFile"
                onChange={e =>
                  setimage({ ...image, selectedFile: e.target.files[0] })
                }
              />
              <Button
                variant="contained"
