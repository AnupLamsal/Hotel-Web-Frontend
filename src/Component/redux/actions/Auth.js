...
export const addCart = (cart, formData) => async dispatch => {
  try {
    ...
    const {
      data: { message },
    } = await api.addCart([...cart, { ...formData, quantity: 1 }]);
    NotifySuccess(message);
    dispatch({ type: IS_NOT_LOADING });
  } catch (error) {
    if (error.response.status >= 400 && error.response.status <= 500) {
...
export const deleteaCart =
  ({ id, cart }) =>
  async () => {
    try {
      const {
        data: { message },
      } = await api.deleteaCart(id, cart);
      console.log(message);
...
export const incrementaCart =
  ...
  async () => {
    try {
      ...
      const {
        data: { message },
      } = await api.incrementaCart(id, data);
      NotifySuccess(message);
    } catch (error) {
      if (error.response.status >= 400 && error.response.status <= 500) {
        NotifyError(error.response.data.message);
...
export const reportByUser = (userId, reportData) => async () => {
  try {
    const {
      data: { message },
    } = await api.reportData(userId, { reportData });
    NotifySuccess(message);
  } catch (error) {
    if (error.response.status >= 400 && error.response.status <= 500) {
      NotifyError(error.response.data.message);
